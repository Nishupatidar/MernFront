...
function Autoblog_edit() {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:4000/autoblogedit/${xid.aid}`).then((res)=>{
      console.log(res.data.autoblogedit)
      setautoblog(res.data.autoblogedit)
      setInitialAutoblog(res.data.autoblogedit)
    })
  },[])
  const autoblogsdelete = (id)=>{
      axios.get(`http://localhost:4000/autoblogdelete/${id}`).then((res)=>{
...
// axios.get()
  ...
  const autoblogs = (e)=>{
       ...
       axios.put('http://localhost:4000/autoblogupdate',autoblog).then((res)=>{
        if(res.data.success){
          ...
          <ToastContainer position = 'top-center'/>
                <div className="row " style={{ "marginLeft": "119px", "marginTop": "53px" }}>
                    <form className="form bg-light mt-2 p-5">
                        <h6 className='alert alert-light text-center'>Autoblog/update</h6>
